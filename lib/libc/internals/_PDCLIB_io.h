...
/* Repositions within a file. Returns new offset on success,
   -1 / errno on error.
*/
_PDCLIB_int_fast64_t _PDCLIB_seek(_PDCLIB_file_t* stream, _PDCLIB_int_fast64_t offset, int whence);
...
struct _PDCLIB_fileops {
     ...
     */
    _PDCLIB_bool (*read)(
        _PDCLIB_fd_t self, void* buf, _PDCLIB_size_t length, _PDCLIB_size_t* numBytesRead);
     ...
     */
    _PDCLIB_bool (*write)(
        _PDCLIB_fd_t self, const void* buf, _PDCLIB_size_t length, _PDCLIB_size_t* numBytesWritten);
     ...
     */
    _PDCLIB_bool (*seek)(
        _PDCLIB_fd_t self, _PDCLIB_int_fast64_t offset, int whence, _PDCLIB_int_fast64_t* newPos);
     ...
     */
    _PDCLIB_bool (*wread)(
        _PDCLIB_fd_t self, _PDCLIB_wchar_t* buf, _PDCLIB_size_t length,
        _PDCLIB_size_t* numCharsRead);
     ...
     */
    _PDCLIB_bool (*wwrite)(
        _PDCLIB_fd_t self, const _PDCLIB_wchar_t* buf, _PDCLIB_size_t length,
        _PDCLIB_size_t* numCharsWritten);
};
...
static inline _PDCLIB_size_t
_PDCLIB_getchars(char* out, _PDCLIB_size_t n, int stopchar, _PDCLIB_file_t* stream)
{
    _PDCLIB_size_t i = 0;
    int c;
    while (stream->ungetidx > 0 && i != n) {
        c = (unsigned char)(out[i++] = stream->ungetbuf[--(stream->ungetidx)]);
        if (c == stopchar)
            return i;
    }
...
_PDCLIB_size_t _PDCLIB_fread_unlocked(
    void* ptr, _PDCLIB_size_t size, _PDCLIB_size_t n, struct _PDCLIB_file* stream) _PDCLIB_nothrow;
_PDCLIB_size_t _PDCLIB_fwrite_unlocked(
    const void* ptr, _PDCLIB_size_t size, _PDCLIB_size_t n,
    struct _PDCLIB_file* stream) _PDCLIB_nothrow;
...
int _PDCLIB_fgetpos_unlocked(
    struct _PDCLIB_file* _PDCLIB_restrict stream,
    _PDCLIB_fpos_t* _PDCLIB_restrict pos) _PDCLIB_nothrow;
int _PDCLIB_fsetpos_unlocked(struct _PDCLIB_file* stream, const _PDCLIB_fpos_t* pos)
    _PDCLIB_nothrow;
...
int _PDCLIB_vprintf_unlocked(const char* _PDCLIB_restrict format, _PDCLIB_va_list arg)
    _PDCLIB_nothrow;
int _PDCLIB_fprintf_unlocked(
    struct _PDCLIB_file* _PDCLIB_restrict stream, const char* _PDCLIB_restrict format,
    ...) _PDCLIB_nothrow;
int _PDCLIB_vfprintf_unlocked(
    struct _PDCLIB_file* _PDCLIB_restrict stream, const char* _PDCLIB_restrict format,
    _PDCLIB_va_list arg) _PDCLIB_nothrow;
...
int _PDCLIB_vscanf_unlocked(const char* _PDCLIB_restrict format, _PDCLIB_va_list arg)
    _PDCLIB_nothrow;
int _PDCLIB_fscanf_unlocked(
    struct _PDCLIB_file* _PDCLIB_restrict stream, const char* _PDCLIB_restrict format,
    ...) _PDCLIB_nothrow;
int _PDCLIB_vfscanf_unlocked(
    struct _PDCLIB_file* _PDCLIB_restrict stream, const char* _PDCLIB_restrict format,
    _PDCLIB_va_list arg) _PDCLIB_nothrow;
