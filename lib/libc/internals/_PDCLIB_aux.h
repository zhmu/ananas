...
#if defined(__GNUC__)
#define _PDCLIB_GCC_MIN(maj, min) ((__GNUC__ > maj) || (__GNUC__ == maj && __GNUC_MINOR__ >= min))
#else
...
#else
#define _PDCLIB_C_MINMAX(min, max) (_PDCLIB_C_VERSION >= (min) && _PDCLIB_C_VERSION <= (max))
#define _PDCLIB_CXX_MINMAX(min, max) (_PDCLIB_CXX_VERSION >= (min) && _PDCLIB_CXX_VERSION <= (max))
#define _PDCLIB_XOPEN_MINMAX(min, max) \
    (defined(_XOPEN_SOURCE) && _XOPEN_SOURCE >= (min) && _XOPEN_SOURCE <= (max))
#define _PDCLIB_POSIX_MINMAX(min, max) \
    (defined(_POSIX_C_SOURCE) && _POSIX_C_SOURCE >= (min) && _POSIX_C_SOURCE <= (max))
