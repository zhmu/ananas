...
size_t strftime(char* s, size_t maxsize, const char* format, const struct tm* tm)
{
    ...
    for (/* nothing */; *format != '\0'; format++) {
        ...
        switch (*format) {
            case 'a':
                append(
                    &st, (tm->tm_wday >= 0 && tm->tm_wday <= 6) ? wday_short[tm->tm_wday] : "???");
                break;
            case 'A':
                append(
                    &st, (tm->tm_wday >= 0 && tm->tm_wday <= 6) ? wday_full[tm->tm_wday] : "???");
                break;
            case 'b':
            case 'h':
            ...
            case 'B':
                ...
                break;
            case 'c': /* Replaced by the locale's appropriate date and time representation. (See the
            ...
            case 'D':
                snprintf(
                    tmp, sizeof(tmp) - 1, "%02d/%02d/%02d", tm->tm_year, tm->tm_mday, tm->tm_mon);
                append(&st, tmp);
                break;
            case 'e':
            ...
            case 'x': /* locale's appropriate date representation */
                snprintf(
                    tmp, sizeof(tmp) - 1, "%d-%02d-%02d", tm->tm_year, tm->tm_mon, tm->tm_mday);
                append(&st, tmp);
                break;
            case 'g': /* Replaced by the last 2 digits of the week-based year (see below) as a
                         decimal number [00,99]. [ tm_year, tm_wday, tm_yday] */
                append(&st, "[todo %g]");
                break;
            case 'G': /* Replaced by the week-based year (see below) as a decimal number (for
            ...
            case 'r': {
                ...
                if (h == 0)
                    h = 12;
                snprintf(
                    tmp, sizeof(tmp) - 1, "%02d:%02d:%02d %s", h, tm->tm_min, tm->tm_sec,
                    am_pm[tm->tm_hour >= 12]);
                append(&st, tmp);
                break;
            }
            ...
            case 'X': /* locale's appropriate time representation */
                snprintf(
                    tmp, sizeof(tmp) - 1, "%02d:%02d:%02d", tm->tm_hour, tm->tm_min, tm->tm_sec);
                append(&st, tmp);
                break;
            case 'u': {
                int wday = tm->tm_wday;
            ...
            case 'U': /* Replaced by the week number of the year as a decimal number [00,53]. The
                         first Sunday of January is the first day of week 1; days in the new year
                         before this are in week 0. [ tm_year, tm_wday, tm_yday] */
                append(&st, "[todo %U]");
                break;
            case 'V': /* Replaced by the week number of the year (Monday as the first day of the
                         ...
                         it is the last week of the previous year, and the next week is week 1. Both
                         January 4th and the first Thursday of January are always in week 1. [
            ...
            case 'w':
                ...
                append(&st, tmp);
                break;
            case 'W': /* Replaced by the week number of the year as a decimal number [00,53]. The
