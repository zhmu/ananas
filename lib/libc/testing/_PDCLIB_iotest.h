...
#define PRINTF_TEST(expected_rc, expected_string, ...)                                     \
    do {                                                                                   \
        ...
        GET_RESULT                                                                         \
        if ((actual_rc != expected_rc) || (RESULT_MISMATCH(target, expected_string))) {    \
            ++TEST_RESULTS;                                                                \
            fprintf(                                                                       \
                stderr,                                                                    \
                ...
                "        expected %2d, \"%s\"\n"                                           \
                "        actual   %2d, \"%s\"\n",                                          \
                __LINE__, GETFMT(__VA_ARGS__, 0), expected_rc, expected_string, actual_rc, \
...
#define PREPARE_SOURCE(input_string)                       \
    ...
    rewind(source);
#elif defined(_PDCLIB_STRINGIO)
#define PREPARE_SOURCE(input_string) memcpy(source, input_string, sizeof(input_string));
#endif
...
#define SCANF_TEST(expected_rc, input_string, ...)                         \
    do {                                                                   \
        ...
        if (actual_rc != expected_rc) {                                    \
            ...
            fprintf(                                                       \
                stderr,                                                    \
                "FAILED: " __FILE__ " (" _PDCLIB_FILEID                    \
