...
static char** argvToAnsi(wchar_t** wargv, int argc)
{
    ...
    for (int i = 0; i != argc; ++i) {
        int sz = WideCharToMultiByte(
            CP_ACP, WC_COMPOSITECHECK | WC_SEPCHARS, wargv[i], -1, NULL, 0, NULL, NULL);
        if (!(argv[i] = malloc(sz))) {
            fputs(
                "Error in C runtime initialization: "
                "unable to allocate buffer for argument",
                stderr);
            abort();
        }

        int rv = WideCharToMultiByte(
            CP_ACP, WC_COMPOSITECHECK | WC_SEPCHARS, wargv[i], -1, argv[i], sz, NULL, NULL);

        if (rv != sz) {
            fputs(
                "Error in C runtime initialization: "
                "size mismatch during character set conversion",
                stderr);
            abort();
        }
    }
...
void __cdecl mainCRTStartup(void)
{
    ...
    if (tss_create(&_PDCLIB_locale_tss, (tss_dtor_t)freelocale) != thrd_success) {
        fputs(
            "Error during C runtime initialization: "
            "Unable to allocate locale TLS",
            stderr);
        exit(EXIT_FAILURE);
    }
    ...
    if (mtx_init(&stdin->lock, mtx_recursive) != thrd_success ||
        ...
        fputs(
            "Error during C runtime initialization: "
            "Unable to allocate stdio mutex",
            stderr);
        exit(EXIT_FAILURE);
    }
