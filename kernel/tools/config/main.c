...
parse_configfile(const char* fname)
{
	...
	while (fgets(line, sizeof(line), f) != NULL) {
		...
		switch (token) {
			...
			case INPUT:
				if (input_dev != NULL)
					errx(1, "%s:%u: input re-specified (was %s)", fname, lineno, input_dev);
				input_dev = strdup(value);
				break;
			case OUTPUT:
				if (output_dev != NULL)
					errx(1, "%s:%u: output re-specified (was %s)", fname, lineno, output_dev);
				output_dev = strdup(value);
				break;
			case HINTS:
				if (hints != NULL)
...
create_console()
{
	...
	if (input_dev != NULL)
		dissect_device_bus(input_dev, &in_dev, &in_bus);
	if (output_dev != NULL)
		dissect_device_bus(output_dev, &out_dev, &out_bus);
	...
	fprintf(f, "/* This file is automatically generated by config - do not edit! */\n\n");
	if (in_dev != NULL)
		fprintf(f, "#define CONSOLE_INPUT_DRIVER drv_%s\n", in_dev);
	else
		fprintf(f, "/* #define CONSOLE_INPUT_DRIVER */\n");
	if (in_bus != NULL)
		fprintf(f, "#define CONSOLE_INPUT_BUS \"%s\"\n", in_bus);
	else
		fprintf(f, "/* #define CONSOLE_INPUT_BUS */\n");
	if (out_dev != NULL)
		fprintf(f, "#define CONSOLE_OUTPUT_DRIVER drv_%s\n", out_dev);
	else
		fprintf(f, "/* #define CONSOLE_OUTPUT_DRIVER */\n");
	if (out_bus != NULL)
		fprintf(f, "#define CONSOLE_OUTPUT_BUS \"%s\"\n", out_bus);
	else
		fprintf(f, "/* #define CONSOLE_OUTPUT_BUS */\n");
...
main(int argc, char* argv[])
{
	...
	if (input_dev == NULL)
		warnx("%s: no input device specified; things will be static", argv[0]);
	if (output_dev == NULL)
		warnx("%s: no output device specified; things will be quiet", argv[0]);
	if (hints == NULL)
