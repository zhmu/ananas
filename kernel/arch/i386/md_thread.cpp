...
md_thread_init(thread_t* t, int flags)
{
	...
	if ((flags & THREAD_ALLOC_CLONE) == 0) {
		vmarea_t* va;
		errorcode_t err = vmspace_mapto(t->t_process->p_vmspace, USERLAND_STACK_ADDR, 0, THREAD_STACK_SIZE, VM_FLAG_USER | VM_FLAG_READ | VM_FLAG_WRITE | VM_FLAG_ALLOC | VM_FLAG_MD, &va);
...
thread_t*
md_thread_switch(thread_t* new_thread, thread_t* old_thread)
{
	...
	KASSERT(THREAD_IS_ACTIVE(new_thread), "new_thread thread isn't running?");

	/* XXX Safety nets to ensure we won't restore a bogus stack */
	KASSERT(new_thread->md_esp > (addr_t)new_thread->md_kstack, "new_thread=%p(%s) esp %p underflow (%p)", new_thread, new_thread->t_threadinfo->ti_args, new_thread->md_esp, new_thread->md_kstack);
	KASSERT(new_thread->md_esp <= ((addr_t)new_thread->md_kstack + KERNEL_STACK_SIZE), "new_thread=%p esp %p overflow (%p)", new_thread, new_thread->md_esp, new_thread->md_kstack + KERNEL_STACK_SIZE);

	/* Activate the corresponding kernel stack in the TSS */
	struct TSS* tss = (struct TSS*)PCPU_GET(tss);
	tss->esp0 = new_thread->md_esp0;

	/* Set debug registers */
	DR_SET(0, new_thread->md_dr[0]);
	DR_SET(1, new_thread->md_dr[1]);
	DR_SET(2, new_thread->md_dr[2]);
	DR_SET(3, new_thread->md_dr[3]);
	DR_SET(7, new_thread->md_dr[7]);

	/* Switch to the new_thread page table */
	__asm __volatile("movl %0, %%cr3" : : "r" (new_thread->md_cr3));

	/*
	 * Switch to the new_thread thread; as this will only happen in kernel -> kernel
	 ...
	 *
	 * %ebx is hardcoded to be the old_thread thread upon entry and return (it won't be
	...
	__asm __volatile(
		"pushfl\n"
		"pushl %%ebp\n"
		"movl %%esp,%[old_esp]\n" /* store old_thread %esp */
		"movl %[new_esp], %%esp\n" /* write new_thread %esp */
		"movl $1f, %[old_eip]\n" /* write next %eip for old_thread thread */
		"pushl %[new_eip]\n" /* load next %eip for new_thread thread */
		"ret\n"
	"1:\n" /* we are back */
		"popl %%ebp\n"
		"popfl\n"
	: /* out */
		[old_esp] "=m" (old_thread->md_esp), [old_eip] "=m" (old_thread->md_eip), "=b" (prev),
		/* clobbered registers */
		"=c" (ecx), "=d" (edx), "=S" (esi), "=D" (edi)
	: /* in */
		[new_eip] "a" (new_thread->md_eip), "b" (old_thread), [new_esp] "c" (new_thread->md_esp)
	: /* clobber */"memory"
	);
	return prev;
}
