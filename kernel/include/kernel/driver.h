...
{
  public:
    Driver(const char* name, int priority = 1000) : d_Name(name), d_Priority(priority) {}
    virtual ~Driver() = default;

    virtual Device* CreateDevice(const CreateDeviceProperties& cdp) = 0;
    virtual const char* GetBussesToProbeOn() const = 0;

    virtual ConsoleDriver* GetConsoleDriver() { return nullptr; }
...
struct RegisterDriver : init::OnInit {
    RegisterDriver()
        : OnInit(init::SubSystem::Driver, init::Order::Middle, []() {
