...
VMPage&
Allocate(VMArea& va, INode* inode, off_t offset, int flags)
{
...
void VMPage::AssertLocked()
{
	...
	KASSERT(((vp_flags & VM_PAGE_FLAG_LINK) == 0) || ((vp_link->vp_flags & VM_PAGE_FLAG_LINK) == 0), "%p links to linked page %p", this, vp_link);
}

VMPage&
VMPage::Link(VMArea& va)
{
  AssertLocked();
...
void
VMPage::Map(VMSpace& vs, VMArea& va)
{
...
void
VMPage::Dump(const char* prefix) const
{
  kprintf("%s%p: refcount %d vaddr %p flags %s/%s/%s/%c ",
    prefix, this, vp_refcount,
    vp_vaddr,
