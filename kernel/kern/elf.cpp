...
namespace
{
    ...
    static Result read_data(DEntry& dentry, void* buf, off_t offset, size_t len)
    {
        struct VFS_FILE f;
        memset(&f, 0, sizeof(f));
        f.f_dentry = &dentry;
    ...
    static Result LoadEhdr(DEntry& dentry, Elf64_Ehdr& ehdr)
    {
...
struct ELF64Loader final : IExecutor {
  ...
  private:
    ...
    Result LoadFile(
        VMSpace& vs, DEntry& dentry, const Elf64_Ehdr& ehdr, addr_t rbase, addr_t& exec_addr) const;
};
