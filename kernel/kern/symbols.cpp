...
symbol_resolve_name(const char* name, struct SYMBOL* s)
{
	Elf_Sym* sym = static_cast<Elf_Sym*>(sym_tab);
	for (unsigned int i = 0; i < sym_tab_size; i += sizeof(Elf_Sym), sym++) {
		if (strcmp(static_cast<char*>(str_tab) + sym->st_name, name) != 0)
			continue;
		s->s_addr = sym->st_value;
		s->s_name = static_cast<char*>(str_tab) + sym->st_name;
		s->s_flags = 0; /* TODO */
...
#ifndef BROKEN
	struct LOADER_MODULE* kernel_mod = static_cast<struct LOADER_MODULE*>(kmem_map(bootinfo->bi_modules, bootinfo->bi_modules_size, VM_FLAG_READ));
