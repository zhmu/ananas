...
struct Impl
{
	static Tag* CreateTag(Tag* parent, int alignment, dma_addr_t min_addr, dma_addr_t max_addr, unsigned int max_segs, dma_size_t max_seg_size)
	{
...
Tag::Tag(Tag* parent, int alignment, dma_addr_t min_addr, dma_addr_t max_addr, int max_segs, dma_size_t max_seg_size)
	: t_parent(parent), t_refcount(1), t_alignment(alignment),
	  t_min_addr(min_addr), t_max_addr(max_addr), t_max_segs(max_addr),
	  t_max_seg_size(max_seg_size)
{
}

Tag*
CreateTag(Tag* parent, int alignment, dma_addr_t min_addr, dma_addr_t max_addr, unsigned int max_segs, dma_size_t max_seg_size)
{
	return detail::Impl::CreateTag(parent, alignment, min_addr, max_addr, max_segs, max_seg_size);
}
...
Result
Tag::AllocateBuffer(dma_size_t size, Buffer*& buf)
{
...
Buffer::Load(void* data, dma_size_t size, dma_load_func_t load, void* load_arg, int flags)
{
	...
	if (phys >= db_tag.t_min_addr && phys + size <= db_tag.t_max_addr &&
	    size < db_tag.t_max_seg_size &&
	    (phys & db_tag.t_alignment) == 0) {
...
Result
Buffer::LoadBIO(struct BIO* bio, dma_load_func_t load, void* load_arg, int flags)
{
