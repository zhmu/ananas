...
void
elf64_add_note(char*& cur, const char* name, size_t contentLen, uint32_t type)
{
	size_t nameLen = strlen(name) + 1 /* terminating \0 */;

	*(uint32_t*)cur = nameLen; cur += sizeof(uint32_t); // namesz
	*(uint32_t*)cur = contentLen; cur += sizeof(uint32_t); // descsz
	*(uint32_t*)cur = type; cur += sizeof(uint32_t); // type

	strcpy(cur, name); cur += nameLen; // name
...
Result
elf64_coredump(Thread* t, struct STACKFRAME* sf, struct VFS_FILE* f)
{
	...
	*(uint64_t*)cur = file_count; cur += sizeof(uint64_t);
	*(uint64_t*)cur = PAGE_SIZE; cur += sizeof(uint64_t);
...
Result
core_dump(Thread* t, struct STACKFRAME* sf)
{
