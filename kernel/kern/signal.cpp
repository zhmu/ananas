...
{
public:
	PendingSignal(const siginfo_t& si)
	: ps_SigInfo(si)
	{
	}
...
Result
QueueSignal(Thread& t, const siginfo_t& siginfo)
{
...
Result
QueueSignal(Thread& t, int signo)
{
	siginfo_t si{};
	si.si_signo = signo;
...
Action*
DequeueSignal(Thread& t, siginfo_t& si)
{
	auto& tsd = t.t_sigdata;
	SpinlockGuard sg(tsd.tsd_lock);
...
void
HandleDefaultSignalAction(const siginfo_t& si)
{
	switch(defaultActionForSignal[si.si_signo - 1]) {
		...
		case DefaultAction::Terminate:
			thread_exit(THREAD_MAKE_EXITCODE(THREAD_TERM_SIGNAL, si.si_signo));
			// NOTREACHED
