...
module_sym_resolve_name(const char* name, struct SYMBOL* s)
{
	...
	LIST_FOREACH(&kernel_modules, kmod, struct KERNEL_MODULE) {
		Elf_Sym* sym = static_cast<Elf_Sym*>(kmod->kmod_symptr);
		for (unsigned int i = 0; i < kmod->kmod_sym_size; i += sizeof(Elf_Sym), sym++) {
			if (strcmp(static_cast<char*>(kmod->kmod_strptr) + sym->st_name, name) != 0)
				continue;
			s->s_addr = sym->st_value;
			s->s_name = static_cast<char*>(kmod->kmod_strptr) + sym->st_name;
			s->s_flags = 0; /* TODO */
			return 1;
		}
	}
...
# error What endianness are you?
	...
	for (unsigned int i = 0; i < elf_sym_tab_size; i += sizeof(Elf_Sym)) {
		Elf_Sym* sym = reinterpret_cast<Elf_Sym*>(static_cast<char*>(sym_ptr) + i);
		...
		switch(ELF32_ST_TYPE(sym->st_info)) {
			case STT_NOTYPE: {
				...
				/* Skip empty symbols */
				const char* sym_name = (const char*)(static_cast<char*>(str_ptr) + sym->st_name);
				if (*sym_name == '\0')
					continue;
				struct SYMBOL s;
			...
			case STT_OBJECT:
			case STT_FUNC: {
				const char* sym_name = static_cast<char*>(str_ptr) + sym->st_name;
	...
	for (unsigned int i = 0; i < ehdr->e_shnum; i++) {
		...
		for (unsigned int n = 0; n < shdr->sh_size; n += sizeof(Elf_Rel), rel++) {
			Elf_Sym* sym = reinterpret_cast<Elf_Sym*>(static_cast<char*>(sym_ptr) + ELF32_R_SYM(rel->r_info) * sizeof(Elf_Sym));
