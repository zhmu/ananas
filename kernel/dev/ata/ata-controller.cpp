...
namespace ata
{
    void ATAController::OnIRQ()
    {
        ...
        if (item->flags & ATA_ITEM_FLAG_ATAPI) {
             ...
             */
            uint16_t len =
                (uint16_t)inb(ata_io + ATA_REG_CYL_HI) << 8 | inb(ata_io + ATA_REG_CYL_LO);
            item->bio->length = len;
        }
    ...
    uint8_t ATAController::ReadStatus() { return inb(ata_io_alt + ATA_REG_ALTSTATUS); }
     ...
     * on success.
     */
    int ATAController::Identify(int unit, ATA_IDENTIFY& identify)
    {
#define TINY_WAIT()                          \
    do {                                     \
...
#define HUGE_WAIT()                              \
    do {                                         \
        ...
        if (cl == 0x14 && ch == 0xeb) {
            ...
            Printf(
                "unit %u does not report recognized type (got 0x%x), assuming disk", unit,
                ch << 8 | cl);
        }
...
#define ATA_DELAY()      \
    ...
    void ATAController::StartPIO(struct ATA_REQUEST_ITEM& item)
    {
        if (item.command != ATA_CMD_PACKET) {
            /* Feed the request to the drive - disk */
            outb(
                ata_io + ATA_REG_DEVICEHEAD,
                0xe0 | (item.unit ? 0x10 : 0) | ((item.lba >> 24) & 0xf));
    ...
    void ATAController::StartDMA(struct ATA_REQUEST_ITEM& item)
    {
    ...
    void ATAController::Enqueue(void* request)
    {
        ...
        /*
         * Grab an item from our freelist and use it to store the request; this ensures the caller
         * doesn't need to allocate requests etc (XXX maybe that would be nicer?)
         */
        spinlock_lock(&spl_freelist);
        ...
        for (int unit = 0; unit < 2; unit++) {
            ...
            if (cmd == ATA_CMD_IDENTIFY) {
                sub_device = device_manager::CreateDevice(
                    "atadisk", CreateDeviceProperties(*this, resourceSet));
                ...
                switch (dev_type) {
                    case ATA_TYPE_CDROM:
                        sub_device = device_manager::CreateDevice(
                            "atacd", CreateDeviceProperties(*this, resourceSet));
                        if (sub_device != nullptr)
    ...
    namespace
    {
        struct ATAController_Driver : public Driver {
            ATAController_Driver() : Driver("atacontroller") {}
