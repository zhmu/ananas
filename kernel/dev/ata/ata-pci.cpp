...
namespace ata
{
    Result ATAPCI::Attach()
    {
        ...
        } resources[] = {{0x1f0, 0x3f4, 14}, {0x170, 0x374, 15}, {0, 0, 0}};
        ...
        for (Resource* r = &resources[0]; r->io != 0; r++) {
            ...
            auto atacl = static_cast<ata::ATAController*>(
                device_manager::CreateDevice("atacontroller", CreateDeviceProperties(*this, rs)));
            if (atacl == nullptr)
                continue;
        ...
        struct ATAPCI_Driver : public Driver {
            ...
            Device* CreateDevice(const CreateDeviceProperties& cdp) override
            {
                ...
                if (PCI_CLASS(classrev) != PCI_CLASS_STORAGE ||
                    PCI_SUBCLASS(classrev) != PCI_SUBCLASS_IDE)
