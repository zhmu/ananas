...
void* AcpiOsMapMemory(ACPI_PHYSICAL_ADDRESS PhysicalAddress, ACPI_SIZE Length)
{
    ...
    LIST_FOREACH(&acpi_mappings, amm, struct ACPI_MEMORY_MAPPING)
    {
        ...
        ACPI_DEBUG(
            "FOUND va %p, pa %p..%p matches %p..%p, reffing (to %d), returning %p\n", amm->amm_virt,
            pa, pa + size * PAGE_SIZE, amm->amm_phys, amm->amm_phys + amm->amm_length,
            amm->amm_refcount + 1, va);
...
void AcpiOsUnmapMemory(void* LogicalAddress, ACPI_SIZE Length)
{
    ...
    LIST_FOREACH(&acpi_mappings, amm, struct ACPI_MEMORY_MAPPING)
    {
        ...
        ACPI_DEBUG(
            " pa %p..%p matches %p..%p, dereffing", LogicalAddress, (addr_t)LogicalAddress + Length,
            amm->amm_virt, amm->amm_virt + amm->amm_length * PAGE_SIZE);
    ...
    panic(
        "AcpiOsUnmapMemory(): mapping %p..%p not found", LogicalAddress,
        (addr_t)LogicalAddress + Length);
}
