...
namespace
{
    constexpr uint16_t ADRToDeviceNumber(uint32_t adr) { return (adr >> 16) & 0xffff; }
    ...
    void WalkPCIResourceTable(const ACPI_BUFFER& prt, Func f)
    {
        ...
        for (auto entry = static_cast<const ACPI_PCI_ROUTING_TABLE*>(prt.Pointer);
             entry->Length != 0; entry = reinterpret_cast<const ACPI_PCI_ROUTING_TABLE*>(
                                     reinterpret_cast<const char*>(entry) + entry->Length)) {
            if (f(*entry))
                break;
        }
    }
    ...
    struct ACPI_PCIHB : public Device, private IDeviceOperations, private IBusOperations {
        ...
        Result Attach() override
        {
            ...
            if (ACPI_STATUS status = AcpiGetIrqRoutingTable(acpiHandle, &pcihb_PRT);
                ACPI_FAILURE(status)) {
                Printf(
                    "could not get PCI interrupt routing table: %s", AcpiFormatException(status));
            }
    ...
    struct ACPI_PCIHB_Driver : public Driver {
        ACPI_PCIHB_Driver() : Driver("acpi-pcihb") {}
