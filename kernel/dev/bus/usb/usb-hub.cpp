...
usbhub_int_callback(struct USB_PIPE* pipe)
{
	...
	auto usb_hub = static_cast<struct USB_HUB*>(usb_dev->usb_device->privdata);
...
usbhub_handle_explore_new_device(struct USB_DEVICE* hub_dev, struct HUB_PORT* port, int n)
{
	...
	auto hub_hub = static_cast<struct USB_HUB*>(hub_dev->usb_device->privdata);
...
usbhub_handle_explore(struct USB_DEVICE* usb_dev)
{
	...
	auto hub = static_cast<struct USB_HUB*>(dev->privdata);
...
usbhub_attach(device_t dev)
{
	...
	auto hub = static_cast<struct USB_HUB*>(kmalloc(sizeof(struct USB_HUB) + sizeof(struct HUB_PORT) * hd.hd_numports));
...
usbhub_detach(device_t dev)
{
	...
	auto hub_hub = static_cast<struct USB_HUB*>(dev->privdata);
