...
namespace usb
{
    namespace uhci
    {
        ...
        namespace
        {
            addr_t GetPhysicalAddress(HCD_TD* td)
            {
                if (td == nullptr)
                    return TD_LINKPTR_T;
            ...
            addr_t GetPhysicalAddress(HCD_QH* qh)
            {
                return qh->qh_buf->GetSegments().front().s_phys;
            }

            void DumpTD(HCD_TD* tdd)
            {
                if (tdd == nullptr)
                    return;
                ...
                kprintf(
                    "td [hcd %p td %p] => linkptr [hcd %p td %x] status 0x%x "
                    "[%c%c%c%c%c%c%c%c%c%c%c]",
                    td, &tdd->td_td, tdd->td_next, td->td_linkptr, td->td_status,
                    (td_status & TD_STATUS_SPD) ? 'S' : '.', (td_status & TD_STATUS_LS) ? 'L' : '.',
                ...
                kprintf(
                    " token 0x%x (data%u,maxlen=%u,endpt=%u,addr=%u)) buffer 0x%x\n", td->td_token,
                    (td->td_token & TD_TOKEN_DATA) ? 1 : 0, (td->td_token >> 21) + 1,
                    (td->td_token >> 15) & 0xf, (td->td_token >> 8) & 0x7f, td->td_buffer);
                if (td->td_linkptr & QH_PTR_T)
                    return;
                DumpTD(tdd->td_next);
            }

            void DumpQH(HCD_QH* qhh)
            {
                if (qhh == NULL)
                    return;
                ...
                kprintf(
                    "qh [hcd %p qh %p]  => headptr [ hcd %p qh 0x%x ] elemptr [ hcd %p qh %p]\n",
                    qhh, qh, qhh->qh_next_qh, qh->qh_headptr, qhh->qh_first_td, qh->qh_elementptr);
            ...
            bool VerifyChainAndCalculateLength(HCD_TD* td, int* length)
            {
                int errors = 0;
                *length = 0;
                for (/* nothing */; td != NULL; td = td->td_next) {
                    ...
                    if (len != TD_ACTUALLEN_NONE)
                        *length += len;
                    if (td->td_td.td_status &
                        (TD_STATUS_STALLED | TD_STATUS_DATABUFERR | TD_STATUS_BABBLE |
                         TD_STATUS_NAK | TD_STATUS_CRCTOERR | TD_STATUS_BITSTUFF))
                        errors++;
                }
                return errors == 0;
            }
            ...
            HCD_TD* CreateDataTDs(
                UHCI_HCD& hcd, addr_t data, size_t size, int max_packet_size, int token, int ls,
                int token_addr, struct HCD_TD* link_td, struct HCD_TD** last_td)
            {
                int num_datapkts = (size + max_packet_size - 1) / max_packet_size;
                addr_t cur_data_ptr = data + size;

                /*
                 * Data packets are alternating, in DATA1/0/1/0/... fashion; note that we build them
                 * in reverse.
                 */
                int data_token = num_datapkts % 2;
                for (int i = 0; i < num_datapkts; i++) {
                    int xfer_chunk_len = (i == 0) ? (size % max_packet_size) : max_packet_size;
                    if (size == max_packet_size) /* XXX ugly fix because max_packet_size %
                                                    max_packet_size = 0 */
                    ...
                    td_data->td_td.td_linkptr =
                        TO_REG32(TD_LINKPTR_VF | GetPhysicalAddress(link_td));
                    td_data->td_td.td_status =
                        TO_REG32(ls | TD_STATUS_ACTIVE | TD_STATUS_INTONERR(3));
                    td_data->td_td.td_token = TO_REG32(
                        token_addr | TD_TOKEN_MAXLEN(xfer_chunk_len) | TD_TOKEN_PID(token) |
                        (data_token ? TD_TOKEN_DATA : 0));
    ...
    uhci::HCD_TD* UHCI_HCD::AllocateTD()
    {
        dma::Buffer* dma_buffer;
        if (auto result = d_DMA_tag->AllocateBuffer(sizeof(struct uhci::HCD_TD), dma_buffer);
            result.IsFailure())
            return nullptr;
    ...
    uhci::HCD_QH* UHCI_HCD::AllocateQH()
    {
        dma::Buffer* dma_buffer;
        if (auto result = d_DMA_tag->AllocateBuffer(sizeof(struct uhci::HCD_QH), dma_buffer);
            result.IsFailure())
            return nullptr;
...
uhci_free_td(device_t dev, struct HCD_TD* td)
{
    ...
    void UHCI_HCD::Dump()
    {
        ...
        KASSERT(
            (addr_t)uhci_framelist == PTOKV(flbase),
            "framelist %p not in framelist base register %p?", uhci_framelist, PTOKV(flbase));
    ...
    irq::IRQResult UHCI_HCD::OnIRQ()
    {
        ...
        if (stat & UHCI_USBSTS_USBINT) {
             ...
             * up that finished.
             */
            for (auto it = uhci_scheduled_items.begin(); it != uhci_scheduled_items.end();
                 /* nothing */) {
                auto& si = *it;
                ++it;
    ...
    Result UHCI_HCD::SetupTransfer(Transfer& xfer)
    {
        /*
    ...
    Result UHCI_HCD::TearDownTransfer(Transfer& xfer)
    {
    ...
    Result UHCI_HCD::CancelTransfer(Transfer& xfer)
    {
    ...
    Result UHCI_HCD::ScheduleControlTransfer(Transfer& xfer)
    {
        ...
        td_handshake->td_td.td_status =
            TO_REG32(ls | TD_STATUS_IOC | TD_STATUS_ACTIVE | TD_STATUS_INTONERR(3));
        td_handshake->td_td.td_token =
            TO_REG32(token_addr | TD_TOKEN_PID(isread ? TD_PID_OUT : TD_PID_IN) | TD_TOKEN_DATA);
        ...
        if (xfer.t_flags & TRANSFER_FLAG_DATA) {
            next_setup_ptr = uhci::CreateDataTDs(
                *this, KVTOP((addr_t)&xfer.t_data[0]) /* XXX64 */, xfer.t_length,
                xfer.t_device.ud_max_packet_sz0, isread ? TD_PID_IN : TD_PID_OUT, ls, token_addr,
                td_handshake, nullptr);
        } else {
        ...
        td_setup->td_td.td_linkptr =
            TO_REG32(TD_LINKPTR_VF | uhci::GetPhysicalAddress(next_setup_ptr));
        td_setup->td_td.td_status = TO_REG32(ls | TD_STATUS_ACTIVE | TD_STATUS_INTONERR(3));
        td_setup->td_td.td_token = TO_REG32(
            TD_TOKEN_MAXLEN(sizeof(struct USB_CONTROL_REQUEST)) | token_addr |
            TD_TOKEN_PID(TD_PID_SETUP));
    ...
    Result UHCI_HCD::ScheduleInterruptTransfer(Transfer& xfer)
    {
        int ls = (xfer.t_device.ud_flags & USB_DEVICE_FLAG_LOW_SPEED) ? TD_STATUS_LS : 0;
        uint32_t token_addr = TD_TOKEN_ENDPOINT(xfer.t_endpoint) | TD_TOKEN_ADDRESS(xfer.t_address);
        int isread = xfer.t_flags & TRANSFER_FLAG_READ;
        ...
        uhci::HCD_TD* last_td;
        auto td_chain = uhci::CreateDataTDs(
            *this, KVTOP((addr_t)&xfer.t_data[0]) /* XXX64 */, xfer.t_length,
            xfer.t_device.ud_max_packet_sz0, isread ? TD_PID_IN : TD_PID_OUT, ls, token_addr,
            nullptr, &last_td);
        ...
        uhci_qh_interrupt[index]->qh_qh.qh_elementptr =
            TO_REG32(uhci::GetPhysicalAddress(td_chain));

        return Result::Success();
    }

    Result UHCI_HCD::ScheduleTransfer(Transfer& xfer)
    {
        auto& usb_dev = xfer.t_device;
        usb_dev.AssertLocked();
...
#undef UHCI_HOOK_QH
        ...
        for (int i = 0; i < UHCI_FRAMELIST_LEN; i++) {
            ...
            uhci_framelist[i] =
                TO_REG32(TD_LINKPTR_QH | uhci::GetPhysicalAddress(uhci_qh_interrupt[index]));
        }
        ...
        uhci_Resources.Write4(
            UHCI_REG_FLBASEADD, TO_REG32(uhci_framelist_buf->GetSegments().front().s_phys));
        ...
        if (auto result = GetBusDeviceOperations().AllocateIRQ(*this, 0, *this); result.IsFailure())
            return result;
        uhci_Resources.Write2(
            UHCI_REG_USBINTR,
            UHCI_USBINTR_SPI | UHCI_USBINTR_IOC | UHCI_USBINTR_RI | UHCI_USBINTR_TOCRC);
        delay(10);

        return Result::Success();
    }

    Result UHCI_HCD::Detach()
    {
        panic("detach");
        return Result::Success();
    }

    void UHCI_HCD::SetRootHub(usb::USBDevice& dev)
    {
        ...
        struct UHCI_Driver : public Driver {
            ...
            Device* CreateDevice(const CreateDeviceProperties& cdp) override
            {
                ...
                if (PCI_CLASS(classrev) == PCI_CLASS_SERIAL &&
                    PCI_SUBCLASS(classrev) == PCI_SUBCLASS_USB && PCI_PROGINT(classrev) == 0)
