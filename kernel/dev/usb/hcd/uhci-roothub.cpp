			...
			if (port >= 1 && port <= rh_numports) {
				...
				switch(req->req_value) {
					case HUB_FEATURE_PORT_RESET: {
						...
						rh_Resources.Write2(reg, MASK(rh_Resources.Read2(reg)) | UHCI_PORTSC_RESET);
						...
						for (/* nothing */; n > 0; n--) {
							...
							if (stat & (UHCI_PORTSC_PECHANGE | UHCI_PORTSC_CSCHANGE)) {
								...
								rh_Resources.Write2(port, MASK(rh_Resources.Read2(port)) | (UHCI_PORTSC_PECHANGE | UHCI_PORTSC_CSCHANGE));
								continue;
							}
							...
							rh_Resources.Write2(reg, MASK(rh_Resources.Read2(reg)) | UHCI_PORTSC_PORTEN);
						}
						if (n == 0) {
			...
			if (port >= 1 && port <= rh_numports) {
				...
				switch(req->req_value) {
					...
					case HUB_FEATURE_C_PORT_CONNECTION:
						...
						rh_Resources.Write2(reg, MASK(rh_Resources.Read2(reg)) | UHCI_PORTSC_CSCHANGE);
						break;
