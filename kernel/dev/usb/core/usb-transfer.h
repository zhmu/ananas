...
namespace usb
{
    ...
    namespace internal
    {
        template<typename T>
        struct PendingNode {
            static typename util::List<T>::Node& Get(T& t) { return t.t_Pending; }
        };

        template<typename T>
        struct CompletedNode {
            static typename util::List<T>::Node& Get(T& t) { return t.t_Completed; }
        };

        template<typename T>
        using TransferPendingNodeAccessor =
            typename util::List<T>::template nodeptr_accessor<PendingNode<T>>;
        template<typename T>
        using TransferCompletedNodeAccessor =
            typename util::List<T>::template nodeptr_accessor<CompletedNode<T>>;

    } // namespace internal

    typedef util::List<Transfer, internal::TransferPendingNodeAccessor<Transfer>>
        PendingTransferList;
    typedef util::List<Transfer, internal::TransferCompletedNodeAccessor<Transfer>>
