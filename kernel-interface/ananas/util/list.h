...
namespace util
{
    ...
    {
        template<typename T, typename Accessor>
        using list_type = typename util::List<T, Accessor>;
        ...
        struct list_node {
          ...
          private:
            ...
            template<typename U, typename NodeGetter>
            friend class nodeptr_accessor;
        ...
        struct base_list_iterator {
            ...
            bool operator!=(const base_list_iterator& rhs) const { return !(*this == rhs); }
        ...
        template<typename T, typename Accessor>
        using list_iterator = base_list_iterator<T, Accessor, forward_advance<T, Accessor>>;
        template<typename T, typename Accessor>
        using list_reverse_iterator =
            base_list_iterator<T, Accessor, backward_advance<T, Accessor>>;

        template<typename T, typename NodeGetter>
        struct nodeptr_accessor {
            static T*& Prev(T& t) { return NodeGetter::Get(t).p_Prev; }

            static T*& Next(T& t) { return NodeGetter::Get(t).p_Next; }
        };

        // Yields the member of struct NodePtr that is used for our pointers
        template<typename T>
        struct GetDerivedNodePtr {
            static list_node<T>& Get(T& t) { return t.np_NodePtr; }
        };

        // Default accessor just uses the embedded NodePtr in each node, obtained by inheritance
        template<typename T>
        using default_accessor = nodeptr_accessor<T, GetDerivedNodePtr<T>>;
    ...
    struct List {
        ...
        bool empty() const { return l_Head == nullptr; }

        T& front() { return *l_Head; }

        const T& front() const { return *l_Head; }

        T& back() { return *l_Tail; }

        const T& back() const { return *l_Tail; }

        constexpr List() : l_Head(nullptr), l_Tail(nullptr) {}
        ...
        iterator begin() { return iterator(*this, l_Head); }

        reverse_iterator rbegin() { return reverse_iterator(*this, l_Tail); }
