...
namespace util
{
    ...
    namespace detail
    {
        ...
        struct base_vector_iterator {
            ...
            base_vector_iterator(vector<T>& v, size_t pos) : i_vector(&v), i_pos(pos) {}

            constexpr T* operator->() const { return &operator*(); }

            constexpr T& operator*() const { return (*i_vector)[i_pos]; }
            ...
            constexpr base_vector_iterator& operator++() { return operator+=(1); }
            ...
            operator==(const base_vector_iterator& a, const base_vector_iterator& b)
            {
    ...
    struct vector {
        ...
        vector(vector&& v) : v_Items(v.v_Items), v_Capacity(v.v_Capacity), v_Size(v.v_Size)
        {
            v.v_Items = nullptr;
            v.v_Capacity = 0;
            v.v_Size = 0;
        ...
        ~vector() { delete[] v_Items; }

        constexpr size_t size() const { return v_Size; }

        constexpr size_t capacity() const { return v_Capacity; }

        constexpr bool empty() const { return size() == 0; }

        constexpr void clear() { v_Size = 0; }
        ...
        constexpr const value_type& front() const { return *begin(); }

        constexpr value_type& back() { return *(end() - 1); }

        constexpr const value_type& back() const { return *(end() - 1); }

        constexpr value_type& operator[](size_t pos) { return v_Items[pos]; }

        constexpr const value_type& operator[](size_t pos) const { return v_Items[pos]; }
        ...
        constexpr void push_back(const value_type& val) { insert(end(), val); }
        ...
        constexpr void pop_back() { v_Size--; }
