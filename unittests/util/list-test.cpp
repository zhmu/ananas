...
TEST(List, reverse_iterator)
{
	...
	auto rit = l.rbegin();
	EXPECT_EQ(3, rit->value); ++rit;
	EXPECT_EQ(2, rit->value); ++rit;
	EXPECT_EQ(1, rit->value); ++rit;
...
struct GetChildNodes {
	static typename util::List<T>::Node& Get(T& t) {
		return t.e_ChildNodes;
	}
};

template<typename T> using NodesAccessor = typename util::List<T>::template nodeptr_accessor<GetAllNodes<T> >;
template<typename T> using ChildAccessor = typename util::List<T>::template nodeptr_accessor<GetChildNodes<T> >;
...
TEST(List, node_in_multiple_lists)
{
	...
	{
		auto it = nodes.begin();
		EXPECT_EQ(1, it->e_value); ++it;
		EXPECT_EQ(2, it->e_value); ++it;
		EXPECT_EQ(3, it->e_value); ++it;
		EXPECT_EQ(4, it->e_value); ++it;
		EXPECT_EQ(5, it->e_value); ++it;
		EXPECT_EQ(6, it->e_value); ++it;
