...
static int
verify_kernel(uint32_t* ph_offset, size_t* ph_entsize, int* ph_num, uint64_t* entry)
{
...
fetch_phdr(int bits, uint32_t offset)
{
	switch(bits) {
		case 32: {
...
#define PRINT64(v) \
	...
	ELF_DEBUG("loading done, bits=%d, entry=%x:%x, phys=%x:%x .. %x:%x, virt=%x:%x .. %x:%x\n",
	 ri->ri_bits,
	 PRINT64(ri->ri_entry),
