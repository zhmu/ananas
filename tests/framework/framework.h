...
#define TEST_EXPECT_FAILED \
	do { TestFailure(__FILE__, __LINE__); testResult = TR_Failure; } while (0)

#define TEST_ASSERT_FAILED \
	do { TestFailure(__FILE__, __LINE__); TestAbort(__FILE__, __LINE__); return TR_Failure; } while(0)
...
#define EXPECT_EQ(expected, actual) \
	if ((actual) != (expected)) TEST_EXPECT_FAILED
...
#define ASSERT_EQ(expected, actual) \
	if ((actual) != (expected)) TEST_ASSERT_FAILED
